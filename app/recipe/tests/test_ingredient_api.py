...
class PublicIngredientAPITests(TestCase):
    ...
    def test_login_required(self):
        """Test that login is required to access object"""
        res = self.client.get(INGREDIENTS_URL)
        self.assertEqual(res.status_code, status.HTTP_401_UNAUTHORIZED)


class PrivateIngredientAPITests(TestCase):
    """Test ingredients with authentication"""
    ...
    def test_ingredients_limited_to_user(self):
        """Tests that the ingredients returned are for authenticated user"""
        user2 = get_user_model().objects.create_user(
            email = 'james@oloko.com',
    ...
    def test_create_ingredient_invalid(self):
        """Tests creating ingredient with invalid payload"""
        payload = {'name': ''}
        res = self.client.post(INGREDIENTS_URL, payload)
